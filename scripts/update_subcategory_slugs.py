#!/usr/bin/env python3
"""Add translated slugs + translationKey to all subcategory _index.md files."""
import os, json
...
def get_slug(en_slug, lang):
    """Get translated slug for a given language. Non-Latin langs keep EN slug."""
    if lang == "en":
        return en_slug
    if lang not in LATIN_LANGS:
        return en_slug  # ja, ko, zh, ar, hi keep EN
    return SLUGS.get(en_slug, {}).get(lang, en_slug)

def update_index_md(filepath, lang, en_slug):
    """Add slug and translationKey to an existing _index.md file."""
    ...
    translated_slug = get_slug(en_slug, lang)
    tkey = f"perfumes-{en_slug}"
    ...
    for line in lines:
        if line.strip() == '---':
            if in_front:
                ...
                if not has_slug:
                    new_lines.append(f'slug: "{translated_slug}"')
                if not has_tkey:
                    new_lines.append(f'translationKey: "{tkey}"')
                front_ended = True
            in_front = not in_front
            new_lines.append(line)
            continue
        
        if in_front and not front_ended:
            if line.startswith('slug:'):
                line = f'slug: "{translated_slug}"'
                has_slug = True
            elif line.startswith('translationKey:'):
                line = f'translationKey: "{tkey}"'
                has_tkey = True
...
ALL_LANGS = list(SECTION_DIRS.keys())
total = 0
url_map = {}  # {lang: {en_slug: translated_slug}}
...
for lang in ALL_LANGS:
    ...
    for folder in sorted(os.listdir(perfume_dir)):
        ...
        if os.path.isdir(os.path.join(perfume_dir, folder)) and os.path.exists(idx_path):
            translated = update_index_md(idx_path, lang, folder)
            url_map[lang][folder] = translated
            count += 1
...
# ===================== UPDATE JSON URLs =====================
with open('/home/ubuntu/hbb/data/categories/perfumes.json', 'r') as f:
    data = json.load(f)

# Build reverse mapping: for each lang+group, map position to EN slug
EN_GROUP_SLUGS = {
...
for lang in data:
    section = SECTION_DIRS.get(lang, "perfumes")
    subcats = data[lang].get("subcategories", {})
    
    for group_key in subcats:
        items = subcats[group_key].get("items", [])
        if group_key in EN_GROUP_SLUGS:
            en_slugs = EN_GROUP_SLUGS[group_key]
            for i, item in enumerate(items):
                if i < len(en_slugs):
                    en_slug = en_slugs[i]
                    translated = get_slug(en_slug, lang)
                    item["url"] = f"/{lang}/{section}/{translated}/"

with open('/home/ubuntu/hbb/data/categories/perfumes.json', 'w') as f:
...
for lang in ["en", "fr", "de", "es", "ja"]:
    section = SECTION_DIRS[lang]
    print(f"\n{lang}:")
    for en_slug in ["women", "floral", "everyday", "floral-fruity"]:
        t = get_slug(en_slug, lang)
        print(f"  /{lang}/{section}/{t}/")
