#!/usr/bin/env python3
"""Generate SEO content for subcategory pages using Claude API"""
...
def get_subcategories():
    ...
    for json_file in DATA_DIR.glob("*.json"):
        ...
        with open(json_file) as f:
            data = json.load(f)
        # Format: {"en": {"slug": {"slug": "...", "emoji": "..."}}}
        if "en" in data:
            for slug, info in data["en"].items():
...
def generate_seo(section, subcat_type, slug, lang, lang_name):
    prompt = f"""Generate SEO content for a {section} subcategory page in {lang_name}.
Section: {section}, Type: {subcat_type}, Slug: {slug}

Return ONLY valid JSON:
{{"seo_title": "50-60 char title", "intro_title": "H1 with emoji", "intro": "400-500 words introduction", "faq": [{{"question": "Q1", "answer": "A1"}}, {{"question": "Q2", "answer": "A2"}}, {{"question": "Q3", "answer": "A3"}}, {{"question": "Q4", "answer": "A4"}}], "seo_bottom": "80-100 word summary"}}
...
def update_file(filepath, seo):
    with open(filepath, 'r') as f:
        content = f.read()
    if 'seo_title:' in content: return False
...
def main():
    ...
    total = len(subcats) * len(LANGS)
    print(f"=== SEO Generation: {len(subcats)} subcats × {len(LANGS)} langs = {total} ===")
    
    progress_file = BASE_DIR / "generation/subcats_seo_progress.json"
    ...
    for sc in subcats:
        for lang, lc in LANGS.items():
            ...
            print(f"[{len(done)+1}/{total}] {lang}/{section_slug}/{sc['slug']}")
            ...
            if seo and update_file(fp, seo):
                ...
                if processed % 20 == 0:
                    ...
                    os.system(f'cd {BASE_DIR} && git add content/ && git commit -m "SEO: {processed} subcats" && git push origin main')
            time.sleep(0.5)
    
    json.dump(list(done), open(progress_file, 'w'))
    if processed > 0:
        os.system(f'cd {BASE_DIR} && git add content/ && git commit -m "SEO: final {processed} subcats" && git push origin main')
